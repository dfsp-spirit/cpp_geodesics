...
int main(int argc, char** argv) {
    ...
    geodesic::Mesh mesh;
    mesh.initialize_mesh_data(surface.vertices, surface.faces); // create internal mesh data structure including edges
    ...
    geodesic::GeodesicAlgorithmDijkstra dijkstra_algorithm(
      &mesh); // simplest approximate algorithm: path only allowed on the edges of the mesh
    unsigned const subdivision_level =
      3; // three additional vertices per every edge in subdivision algorithm
    geodesic::GeodesicAlgorithmSubdivision subdivision_algorithm(
      &mesh, 2); // with subdivision_level=0 this algorithm becomes Dijkstra, with
                 // subdivision_level->infinity it becomes exact

    std::vector<geodesic::GeodesicAlgorithmBase*>
      all_algorithms; // for simplicity, store all possible geodesic algorithms here
    ...
    sources.push_back(
      geodesic::SurfacePoint(&mesh.vertices()[0])); // one source is located at vertex zero
    ...
    for (unsigned index = 0; index < all_algorithms.size(); ++index) {
        geodesic::GeodesicAlgorithmBase* algorithm =
          all_algorithms[index]; // all algorithms are derived from GeodesicAlgorithmBase
        ...
        std::vector<double> max_distance(
          sources.size(),
          0.0); // distance to the furthest vertex that is covered by the given source
        ...
        for (int i = 0; i < max_distance.size(); ++i) {
            std::cout << "distance to the furthest vertex that is covered by source " << i << " is "
                      << max_distance[i] << std::endl;
        }
    }
    return 0;
}