  ...
  IntervalWithStop* one) // intersecting two intervals with up to three intervals in the end
{
    ...
    unsigned int Ninter = 0; // number of the points of the intersection
    ...
    double left = std::max(
      zero->start(),
      one->start()); // define left and right boundaries of the intersection of the intervals
    double right = std::min(zero->stop(), one->stop());

    double good_start[4]; // points of intersection within the (left, right) limits +"left" +
                          // "right"
    good_start[0] = left;
    unsigned int Ngood_start = 1; // number of the points of the intersection

    for (unsigned int i = 0; i < Ninter; ++i) // for all points of intersection
    {
        double x = inter[i];
    ...
    for (unsigned int i = 0; i < Ngood_start - 1; ++i) // for all intervals
    {
    ...
    while (!m_queue.empty()) {
             ...
             ++i) // two possible faces to propagate
        {
            ...
            if (num_propagated) {
                ...
                bool const invert =
                  next_edge->v0()->id() !=
                  edge->v0()->id(); // if the origins coinside, do not invert intervals
  ...
  IntervalWithStop* candidates) // if it is the last interval on the edge
{
    ...
    // important: for the first_interval, this function returns zero only if the new edge is
    // "visible" from the source if the new edge can be covered only after turn_over, the value is
    // negative (-1.0)
  ...
  interval_pointer source_interval) // up to two candidates
{
    ...
    if (!invert) // in this case everything is straighforward, we do not have to invert the
                 // intervals
    {
  ...
  std::vector<SurfacePoint>& path)
{
    ...
    if (best_interval) // if we did not hit the face source immediately
    {
        ...
        while (visible_from_source(path.back()) <
               0) // while this point is not in the direct visibility of some source (if we are
                  // inside the FACE, we obviously hit the source)
        {
